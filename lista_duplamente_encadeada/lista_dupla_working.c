...
*/	
	...
	printf("\n\nAVISO: O PROGRAMA NAO EXIBE O NO CABECA.");
	while(opcao!=6)
	{
...
int imprimir(Tno *le)  //função recursiva pra imprimir a lista.
{  
	int cont=0; 
...
void inserirInicio (int x, Tno **le) //pronto
{
   ...
   (*le)->prox=nova;
	...
	if(nova->prox!=NULL) //resolução do problema de null no ant
	{	
		nova->prox->ant=nova;
	}   
   ...
   Tno *lixo;
	...
	else
	{
		...
		else
		{
		   ...
		   (*le)->prox->prox->ant=NULL; //alterando o ant do elemento depois do removido   
...
void removerFinal (Tno *le)
{
	...
	if(aux->ant==NULL) //caso só tenha um elemento na lista
	{	
		lixo=aux;
