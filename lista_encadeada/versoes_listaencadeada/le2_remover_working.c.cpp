	...
	switch(opcao)
	{
		...
		case 2:
			...
			switch(opRemover)
			{
				...
				case 3:
					imprimir(inicio);
					printf("\nEscolha o conteudo a ser removido: (A lista deve ter pelo menos um no alem do no cabeca)");
					scanf("%d",&busca);
					
					aux->prox=busca_r(busca,inicio); //ja procura o prox, pra remover o prox
